...
addOption() {
  ...
  editQuestion(data: any) {
    ...
    this.http.getAllQuestionAnswerList(this.apiData?.user?.idOrganization,this.cubeFaceId).subscribe((res) => {
      ...
      this.questionFormData.get('questionClueDescription')?.setValue(data?.questionClue);
  ...
  postEditQuestion() {
    ...
    const payload = {
      Data: {
        AnsList:this.payloadData?this.payloadData:[
          {
        ...
        QuestionList:{
            ...
            ColumnNo:this.questionEditData?.rowNo ? this.questionEditData?.rowNo:0,
            Direction:this.questionEditData?.direction ?this.questionEditData?.direction:"across",
