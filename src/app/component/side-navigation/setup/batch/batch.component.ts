...
createBatch(){
 console.log(this.batch_name);
  ...
  const escapedobjHeirarchyBatchesMaster = JSON.stringify(payload.Data.objHeirarchyBatchesMaster);
  const escapedIdOrgHierarchy = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IdOrgHierarchy);
  const escapedBatchName = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.BatchName);
  const escapedIsActive = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IsActive);
  const escapedIdOrganization = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IdOrganization);
  const escapedIdCmsUser = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IdCmsUser);
  const escapedlstCubefaceBatchMaster = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.lstCubefaceBatchMaster);
  const escapeCubesFacesId = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.lstCubefaceBatchMaster[0].CubesFacesId);
  const escapedIdBatch = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.lstCubefaceBatchMaster[0].IdBatch);
  const escapedScheduledDateTime = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.lstCubefaceBatchMaster[0].ScheduledDateTime);
...
editBatch(event:any){
  ...
  const escapedobjHeirarchyBatchesMaster = JSON.stringify(payload.Data.objHeirarchyBatchesMaster);
  const escapedIdBatch = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IdBatch);
  const escapedIdOrgHierarchy = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IdOrgHierarchy);
  const escapedIdOrganization = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IdOrganization);
  const escapedBatchName = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.BatchName);
  const escapedIsActive = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IsActive);
  const escapedIdCmsUser = JSON.stringify(payload.Data.objHeirarchyBatchesMaster.IdCmsUser);
  const escapedCubefaceBatchId = JSON.stringify(payload.Data.lstCubefaceBatchMaster[0].CubefaceBatchId);
  const escapedCubesFacesId = JSON.stringify(payload.Data.lstCubefaceBatchMaster[0].CubesFacesId);
  const escapedIdBatchInner = JSON.stringify(payload.Data.lstCubefaceBatchMaster[0].IdBatch);
  const escapedScheduledDateTime = JSON.stringify(payload.Data.lstCubefaceBatchMaster[0].ScheduledDateTime);
